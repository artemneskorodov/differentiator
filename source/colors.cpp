...
static printing_state_t reset_color     (FILE        *output);
static printing_state_t print_color_code(FILE        *output,
                                         color_t      color,
                                         boldness_t   is_bold,
...
int color_printf(color_t      color,
    ...
    print_color_code(stdin, color, is_bold, background);
...
int print_error(const char *message, ...) {
    print_color_code(stderr, RED_TEXT, BOLD_TEXT, DEFAULT_BACKGROUND);
...
printing_state_t print_color_code(FILE        *output,
                                  color_t      color,
                                  boldness_t   is_bold,
                                  background_t background) {
    fprintf(output, "%s", color_code_start);
    if(is_bold == BOLD_TEXT) {
        fprintf(output, "%s", bold);
    ...
    if(background != DEFAULT_BACKGROUND) {
        ...
        _C_ASSERT(code != NULL, );
        fprintf(output, "%sm", code);
        return PRINTING_SUCCESS;
    }
    return reset_color(output);
}

printing_state_t reset_color(FILE *output){
    if(fprintf(output, "%s0m", color_code_start) <= 0)
